...
const formatTransactionForCSV = (tx, prefix) => {
  ...
  return {
    ...
    [`${prefix}fee`]: tx.fee || '',
    [`${prefix}note`]: tx.note || '',
    [`${prefix}data_quality_issues`]: tx.dataQualityIssues ? tx.dataQualityIssues.join('; ') : ''
  };
};
...
const generateCSVReport = async (matchingResults, runId) => {
  ...
  for (const item of matchingResults.matched) {
    ...
    const exchangeFields = formatTransactionForCSV(item.exchangeTx, 'exchange_');
  ...
  for (const item of matchingResults.conflicting) {
    ...
    if (item.details) {
      const details = [];
      if (!item.details.timestampMatch) details.push(`Timestamp diff: ${item.details.timestampDiffSeconds}s`);
      if (!item.details.quantityMatch) details.push(`Quantity diff: ${item.details.quantityDiffPct.toFixed(2)}%`);
